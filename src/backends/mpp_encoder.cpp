...
namespace {
using vip::alignUp;
using vip::hasSuffixIgnoreCase;
using vip::isRtspUrl;
using vip::startsWithIgnoreCase;
using vip::toLowerAscii;
...
size_t alignedFrameBytes(int horStride, int verStride, PixelFormat format) {
  const size_t alignedHor = static_cast<size_t>(vip::alignUp(horStride, 64));
  const size_t alignedVer = static_cast<size_t>(vip::alignUp(verStride, 64));
...
// 薄包装,沿用原有调用风格;实际拼错误描述走 vip::throwFfmpegError。
void checkAvStatus(int status, const char* message) {
  if (status < 0) {
    vip::throwFfmpegError(message, status);
  }
}
